...
export async function POST(req: NextRequest) {
  try {
    ...
    // Bubble Telnyx / 10DLC failures to the intake toast (never throw into the UI).
    if (!sent.ok) {
      console.error("[request-photos] Telnyx SMS failed:", sent.error)
    ...
    console.error("[request-photos] unexpected failure:", e)
    return NextResponse.json(
      { error: "Could not send photo request text. Check server logs for details." },
      { status: 500 }
    )
  }
}
