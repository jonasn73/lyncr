...
describe("sms reply suggestions", () => {
  ...
  it("builds status follow-up chips from vehicle and job without sending", () => {
    ...
    expect(status?.body).toContain("2011 BMW 128i key")
    expect(status?.body).toContain("any update or change")
    expect(status?.body).not.toContain("AKL")
    expect(status?.body.toLowerCase()).not.toContain("asap")
