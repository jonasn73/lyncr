...
describe("handleTelnyxCallControlVoiceWebhook", () => {
  ...
  it("speak.failed after Available greet dials cell when the Polly retry also fails", async () => {
    // Production: Speak HTTP 200 then call.speak.failed — speak.ended never
    // arrived and the owner cell never rang. First failure retries once on Polly;
    // this test fails that retry too, so recovery must fall through to Dialing.
    fetchMock.mockImplementation(async (url: unknown, opts?: { body?: string }) => {
      const body = opts?.body ? (JSON.parse(String(opts.body)) as { voice?: string }) : {}
      if (String(url).includes("/actions/speak") && /^AWS\.Polly\./i.test(body.voice || "")) {
        return {
          ok: false,
          status: 500,
          json: async () => ({ errors: [{ detail: "polly retry also failed" }] }),
        }
      }
      return { ok: true, json: async () => ({ data: { call_control_id: "cc-outbound-1" } }) }
    })
  ...
  it("speak.failed after Available greet retries once on Polly instead of skipping straight to dial", async () => {
    vi.doMock("@/lib/db", () => ({
      getIncomingRoutingForVoiceWebhook: vi.fn(() =>
        Promise.resolve({
          user_id: "u1",
    ...
    await handleTelnyxCallControlVoiceWebhook({
      data: {
        event_type: "call.speak.failed",
        id: "evt-speak-fail-polly-ok",
        payload: {
          call_control_id: "cc-inbound-greet-fail-2",
          from: "+15551230000",
          to: "+15025571219",
          direction: "incoming",
          client_state: inboundState,
    ...
    // Retry attempt must use the Polly Neural fallback voice, not NaturalHD again.
    const retrySpeakCall = fetchMock.mock.calls.find(
      (c) => String(c[0]).includes("cc-inbound-greet-fail-2") && String(c[0]).includes("/actions/speak")
    )
    expect(retrySpeakCall).toBeTruthy()
    const retryBody = JSON.parse(String(retrySpeakCall![1]?.body || "{}")) as { voice?: string }
    expect(retryBody.voice).toBe("AWS.Polly.Joanna-Neural")
    // The default mock resolves every fetch ok:true, so this retry "succeeds" — the cell
    // must NOT be dialed yet; that only happens once this retried Speak's own
    // call.speak.ended (or a second call.speak.failed) arrives.
    const dialCall = fetchMock.mock.calls.find(
      (c) => String(c[0]).includes("/v2/calls") && !String(c[0]).includes("/actions/")
    )
