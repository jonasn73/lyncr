...
| `LYNCR_CALL_CONTROL_SPEAK_VOICE` | Optional ops override for Call Control Speak / Busy menus. When unset, uses the saved **AI Voice Persona** (default **`Telnyx.NaturalHD.astra`** for Reassuring Female). Examples: `AWS.Polly.Joanna-Neural`, `AWS.Polly.Matthew-Neural`, `Telnyx.NaturalHD.astra`. Legacy `ZING_CALL_CONTROL_SPEAK_VOICE` still works. |
