    ...
    <>
      ...
      {showEmptyState ? (
        <button
          ...
          className={cn(
            "mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-[var(--electric-glow)] transition-[opacity,transform] hover:bg-primary/90 motion-safe:active:scale-[0.98]",
            MOBILE_TAP_TARGET
          )}
        >
      ...
      <nav className="mt-5 flex flex-col gap-1" aria-label="Number shortcuts">
        <button
          ...
          className={cn(
            "group flex w-full items-center justify-between rounded-lg px-3 text-left text-sm font-medium text-foreground transition-colors hover:bg-white/5",
            MOBILE_TAP_TARGET
          )}
        >
