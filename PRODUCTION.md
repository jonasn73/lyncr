...
| `NEXT_PUBLIC_SENTRY_DSN` | Sentry DSN for production crash reports + session replay (text masked, media blocked). Get it from sentry.io → Project → Client Keys. Also accepted as `SENTRY_DSN`. |
| `SENTRY_AUTH_TOKEN` | Optional. Only if you want Vercel builds to upload source maps to Sentry. |
| `CRON_SECRET` | Random secret so only Vercel Cron (and you) can hit `/api/cron/*`. Vercel sends `Authorization: Bearer <CRON_SECRET>`. Generate with `openssl rand -hex 24`. |
