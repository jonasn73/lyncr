...
describe("parseAmberCommand", () => {
  it("parses HELP STATUS AVAILABLE BUSY", () => {
    ...
    expect(parseAmberCommand("What's going on around my business").kind).toBe("briefing")
    expect(parseAmberCommand("What's going around my dashboard?").kind).toBe("briefing")
    expect(parseAmberCommand("catch me up").kind).toBe("briefing")
    expect(parseAmberCommand("tell him what's going on").kind).not.toBe("briefing")
