...
describe("handleTelnyxCallControlVoiceWebhook", () => {
  ...
  it("day_dial + Hold fallback skips the branded greeting and dials the owner directly", async () => {
    // Hold queue plays its own "all of our team members..." greeting once a direct ring to
    // the owner is missed — hearing "Thanks for calling X" first too is a redundant double
    // greeting for this specific combo, even with inbound_caller_greeting_enabled true.
    vi.doMock("@/lib/db", () => ({
      getIncomingRoutingForVoiceWebhook: vi.fn(() =>
        Promise.resolve({
          user_id: "u1",
    ...
    await handleTelnyxCallControlVoiceWebhook({
      data: {
        ...
        id: "evt-answered-hold-skip-greeting",
        payload: {
          call_control_id: "cc-hold-skip-greeting",
          from: "+15025369252",
          to: "+15025571219",
          direction: "incoming",
  ...
  it("day_dial + voicemail fallback still plays the branded greeting (skip is Hold-only)", async () => {
    vi.doMock("@/lib/db", () => ({
      getIncomingRoutingForVoiceWebhook: vi.fn(() =>
        Promise.resolve({
          user_id: "u1",
    ...
    await handleTelnyxCallControlVoiceWebhook({
      data: {
        event_type: "call.answered",
        id: "evt-answered-voicemail-keeps-greeting",
        payload: {
          call_control_id: "cc-voicemail-keeps-greeting",
          from: "+15025369252",
          to: "+15025571219",
          direction: "incoming",
          client_state: answeredState,
