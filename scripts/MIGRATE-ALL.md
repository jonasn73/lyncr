...
| 65 | `065-organizations-external-lines.sql` | **Multi-business workspaces + external lines.** Adds **`organizations`** (one owner → many businesses), links **`phone_numbers.organization_id`**, and **`source_provider` / `external_verified`** for Twilio-forwarded DIDs. Backfills a default org per owner. **Required** for the dashboard business switcher and “Bring an Existing Number” flow. |
