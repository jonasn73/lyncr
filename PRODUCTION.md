...
| `ZING_CALL_CONTROL_SPEAK_VOICE` | Optional. Voice for **Call Control** `speak` / Busy menus (Key Squad production). Default **`AWS.Polly.Joanna-Neural`**. Must use Telnyx Call Control format (`AWS.Polly.*`, `Azure.*`, `Telnyx.NaturalHD.astra`, or `ElevenLabs.…`). Bare `Polly.*` is auto-upgraded to `AWS.Polly.*`. |
