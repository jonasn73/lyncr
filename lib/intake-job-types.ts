...
const INTAKE_LOCKSMITH_JOB_TYPES = [
  "Key replacement",
  "Lockout",
  "Ignition",
  "Other",
...
type KeyReplacementMode = (typeof KEY_REPLACEMENT_MODES)[number]

function isIntakeLocksmithJobType(value: string): value is IntakeLocksmithJobType {
  return (INTAKE_LOCKSMITH_JOB_TYPES as readonly string[]).includes(value)
}

function isKeyReplacementMode(value: string): value is KeyReplacementMode {
