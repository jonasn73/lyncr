...
| 52 | `052-invite-sms-channel.sql` | Extends **`team_invites`** with **`channel`** (`EMAIL`/`SMS`), **`phone`**, and **`status`** (`PENDING`/`ACCEPTED`/`EXPIRED`), and makes `email`/`first_name` optional. Powers the EMAIL **or** SMS receptionist invite flow (admin "Invite receptionist" modal → `/register?token=`). Read defensively, so the existing email-invite flow keeps working before this runs. |
